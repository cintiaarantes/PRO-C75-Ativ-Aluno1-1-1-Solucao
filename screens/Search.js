...
//Aula 73: Importando o componente ListItem
import { Avatar, ListItem, Icon } from "react-native-elements";

//Aula 73: Importando o BD para ter registro do histórico
import db from "../config";

export default class SearchScreen extends Component {
  //Aula 73: Criando uma matriz vazia no estado chamada: allTransactions (ela irá guardar todas as transações)
  constructor(props) {
    super(props);
    this.state = {
      allTransactions: [],
      //Aula 74: Criar o estado de pesquisa "vazio", para permitir uma pesquisa. Estado da última transação visível
      searchText: "",
      lastVisibleTransaction: null,
    };
  }

  //Aula 73: Chamamos dentro de componentDidMount, pois queremos obter os dados assim que o componente for carregado
  componentDidMount = async () => {
    this.getTransactions();
  };

  //Aula 73: Função obtém as transações: Fazendo uma consulta na coleção: 'transactions' e armazená-las na matriz allTransacions
  getTransactions = () => {
    db.collection("transactions")
      .limit(10) //Aula 74: Função limite para limitar para carregarn apenas as 10 últimas transações.
      .get()
      .then((snapshot) => {
  ...
  */
  renderItem = ({ item, i }) => {
    ...
    //Aula 73: Variável que irá verificar o tipo de transação. SE IGUAL "issue" = livro entregue/emprestado, SENÃO: livro devolvido
    var transactionType =
      item.transaction_type === "issue" ? "entregue" : "devolvido";

    //Aula 73: Na função return usando o component ListItem, cria-se uma UI (Interface do Usuário) p/ lista transações
    return (
  ...
  //Aula 73: Função: Gerenciar de Pesquisa, usando o parâmetro de texto ela pesquisa todas as transações registras no banco de dados BD
  ...
  //Aula 74 - Desafio 01: Fazer a função de "Buscar mais transações" usando o carregamento lento (com o limite de 10).
  ...
  FlatList: Usado para listar todas as transações em formato de Lista. Ela tem 3 propriedades principais:
    ...
    Aula 74: FlatList tem outras duas propriedades: que realizam a Lazy Loading (carregamento lento).
